...
#sum of rows
avgV=mean(dataV[0:ns])
avgI=mean(dataI[0:ns])

#rms of rows
rmsV=sqrt(mean(dataV[0:ns]**2))
rmsI=sqrt(mean(dataI[0:ns]**2))
...
for n in range(1,101):
    for m in range(0,ns):
        aI[m,n]=dataI[m]*cos(n*t[m]*2*pi*f)      # Fourier coefficients I
        bI[m,n]=dataI[m]*sin(n*t[m]*2*pi*f)
        aV[m,n]=dataV[m]*cos(n*t[m]*2*pi*f)      # Fourier coefficients V
        bV[m,n]=dataV[m]*sin(n*t[m]*2*pi*f)
...
for n in range(1,101): 
    ...
    for Ti in range (0,ns):
        FI[Ti,n] = CI[n]*cos(n*2*pi*f*t[Ti]+ThetaI[n])  # Calculation of F
        FV[Ti,n] = CV[n]*cos(n*2*pi*f*t[Ti]+ThetaV[n])
    RMSI[n] = sqrt(sum(FI[:-1,n]**2)/ns)                # RMS of each harmonic / Current
    HfI[n] = 100*RMSI[n]/RMSI[1]
    HrmsI[n]=100*RMSI[n]/rmsI
    RMSV[n] = sqrt(sum(FV[:-1,n]**2)/ns)                # RMS of each harmonic / Voltage
    HfV[n] = 100*RMSV[n]/RMSV[1]
    HrmsV[n] = 100*RMSV[n]/rmsV
    #print('%d-%d\n' % (n,Ti))

if (rmsI >= RMSI[1]):
    THDI = 100*sqrt(((rmsI/RMSI[1])**2)-1)                  # Calculation of THD ; current
else:
    THDI = 100*(sqrt(sum((RMSI[2:41])**2))/RMSI[1])

dataHI = sum(FI,1)+avgI

if (rmsV >= RMSV[1]):
    THDV = 100*sqrt(((rmsV/RMSV[1])**2)-1)                    # Calculation of THD ; voltage
else:
    THDV = 100*(sqrt(sum((RMSV[2:41])**2))/RMSV[1])

dataHV = sum(FV,1)+avgV

THC = sqrt(sum(RMSI[2:41]**2))                           # CHECK INDEX 40 OR 41!! Calculation of THC (Total Harmonic Current)
...
Q = RMSI * RMSV * sin(ThetaI-ThetaV)
Qt = sum(Q)                                            # Reactive Power

D = sqrt((St**2)-(Pt**2)-(Qt**2))                          # Distortion Power

Pv = [St,Pt,Qt,D]                                      # Power Vector

distF = RMSI[1]/rmsI                                   # Distortion Factor

dispF = cos(ThetaI[1]-ThetaV[1])                       # Displacement Factor
...
if PhaseDiff5 == 0:
    ...
    print('The 5th Harmonic of the Current ' + leadlag5 + ' the fundamental of the Voltage by %0.2f degrees.' %(PhaseDiff5_R))
